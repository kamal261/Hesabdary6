    ...
    suspend fun restoreBackup(context: Context, uri: Uri, db: AppDatabase): Int =
        withContext(Dispatchers.IO) {
            ...
            if (root.has("categories")) {
                ...
                // Pass 2: subcategories. The backup's own numeric "id"/"parentId" values are
                // meaningless here -- Room assigns fresh autogenerated ids on insert, so each
                // parent is re-resolved by NAME against what's now actually in the database,
                // not by the old id (this is the same class of id-mapping bug the restore
                // confirmation dialog above already warns about for a non-empty database).
                val idToName = entries.associate { it.getLong("id") to it.getString("name") }
                entries.filterNot { it.isNull("parentId") || !it.has("parentId") }.forEach { o ->
                    val name = o.getString("name")
                    if (db.categoryDao().countByName(name) > 0) return@forEach
                    val parentName = idToName[o.getLong("parentId")]
                    val parentId = parentName?.let { pn ->
                        db.categoryDao().getAllOnce().firstOrNull { it.name == pn }?.id
                    }
                    db.categoryDao().insert(
                        Category(
                            name = name,
