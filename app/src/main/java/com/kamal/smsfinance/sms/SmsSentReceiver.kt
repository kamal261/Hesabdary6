...
class SmsSentReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val sentIntent = intent.getParcelableExtra<Intent>("sentIntent")

        when (resultCode) {
            Activity.RESULT_OK -> Log.d(TAG, "SMS sent successfully")
            Telephony.Sms.RESULT_ERROR_GENERIC_FAILURE -> Log.e(TAG, "SMS generic failure")
            Telephony.Sms.RESULT_ERROR_RADIO_OFF -> Log.e(TAG, "SMS radio off")
            Telephony.Sms.RESULT_ERROR_NULL_PDU -> Log.e(TAG, "SMS null PDU")
            Telephony.Sms.RESULT_ERROR_NO_SERVICE -> Log.e(TAG, "SMS no service")
