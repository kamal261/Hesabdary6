 ...
 * 1. Full SHA-256 instead of String.hashCode() or a short digest
 * 2. Sliding window around receivedAt instead of fixed bucket - catches duplicates across boundaries
 * 3. Synchronized check-and-record so the mutable timestamp lists cannot race
 */
object DedupEngine {
    ...
    /**
     * Computes a collision-resistant fingerprint for (sender + body).
     * The full SHA-256 digest is kept because this fingerprint gates financial inserts.
     */
