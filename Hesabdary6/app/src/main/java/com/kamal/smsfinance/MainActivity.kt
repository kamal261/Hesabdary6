...
import com.kamal.smsfinance.data.counterpartyBalance
        ...
        androidx.compose.foundation.layout.Box(modifier = Modifier.padding(padding)) {
            when (tab) {
                ...
                Tab.COUNTERPARTIES -> CounterpartiesScreen(
                    counterparties = counterparties,
                    balanceOf = { id ->
                        // Use the same domain formula as the profile and DAO paths.
                        // The previous income-minus-expense fold ignored debt-settlement categories.
                        transactions
                            .filter { it.counterpartyId == id }
                            .counterpartyBalance { categoryId ->
                                categories.firstOrNull { it.id == categoryId }?.kind
                            }
                    },
