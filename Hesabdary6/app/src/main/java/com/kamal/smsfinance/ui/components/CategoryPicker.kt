...
fun CategoryPicker(
    ...
    allowedKinds: Set<CategoryKind>? = null
) {
    var expanded by remember { mutableStateOf(false) }
    var query by remember { mutableStateOf("") }
    var kindFilter by remember { mutableStateOf<CategoryKind?>(null) }

    val eligibleCategories = remember(categories, allowedKinds) {
        allowedKinds?.let { kinds -> categories.filter { it.kind in kinds } } ?: categories
    }

    val topFour = remember(eligibleCategories, usageCounts) {
        val used = eligibleCategories.filter { (usageCounts[it.id] ?: 0) > 0 }
            .sortedByDescending { usageCounts[it.id] ?: 0 }
        if (used.size >= 4) {
            used.take(4)
        } else {
            val defaults = eligibleCategories.filter { it.isDefault && it !in used }
            (used + defaults).distinct().take(4)
        }
    }

    val filtered = remember(eligibleCategories, query, kindFilter) {
        eligibleCategories.filter { cat ->
    ...
    Column(modifier = modifier) {
        ...
        TextButton(onClick = { expanded = !expanded }) {
            Text(if (expanded) "بستن لیست کامل" else "بیشتر (${eligibleCategories.size} دسته)")
            Icon(
